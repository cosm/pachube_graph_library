...
YUI.qMock.unitTests.singleMethod = new YUI.tool.TestCase({
...
YUI.namespace("qMock.unitTests");
	...
	YUI.qMock.unitTests.singleMethodMultipleExpectations = new YUI.tool.TestCase({
	...
	YUI.qMock.unitTests.multipleParamterlessMethods = new YUI.tool.TestCase({
	...
	YUI.qMock.unitTests.stubbedProperties = new YUI.tool.TestCase({
	...
	YUI.qMock.unitTests.returnValues = new YUI.tool.TestCase({
		...
		testReturnValues: function() {
		    ...
		    YUI.util.ArrayAssert.itemsAreEqual(this.mock.getArrayValue(), [ 1, 2, 3 ], "getArrayValue() should return [ 1, 2, 3 ]");
		    assertEquals(this.mock.getFunctionValue()(), 'function', "getFunctionValue() when invoked should return 'function'");
		    YUI.util.ObjectAssert.propertiesAreEqual(this.mock.getObjectValue(), { id: 5, value: 'value' }, "getObjectValue() should return object");
		    ...
		    YUI.util.ArrayAssert.itemsAreEqual(this.mock.getEmptyArrayValue(), [ ], "getEmptyArrayValue() should return [ ]");
		    YUI.util.ObjectAssert.propertiesAreEqual(this.mock.getEmptyObjectValue(), { }, "getEmptyObjectValue() should return { }");
		    assert(this.mock.verify(), "verify() should be true");
		}
	});
	
	YUI.qMock.unitTests.primitiveArguments = new YUI.tool.TestCase({
	...
	YUI.qMock.unitTests.falseyArguments = new YUI.tool.TestCase({
	...
	YUI.util.Event.onDOMReady(function runTests(){
	    //create the logger
	    var logger = new YUI.tool.TestLogger("testLogger");
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.singleMethod);
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.singleMethodMultipleExpectations);
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.multipleParamterlessMethods);
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.stubbedProperties);
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.returnValues);
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.primitiveArguments);
	    YUI.tool.TestRunner.add(YUI.qMock.unitTests.falseyArguments);
		
	    //run the tests
	    YUI.tool.TestRunner.run();
	});